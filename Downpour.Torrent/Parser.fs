...
let private dictGet (key: string) = function
...
let private parseInfo (infoDict: BencodeValue) : InfoDict =
    {
      ...
      Private     = dictGet "private" infoDict
                    |> Option.map asInt
...
// info hash 
let private computeInfoHash (infoVal: BencodeValue) : InfoHash =
    infoVal
    |> Downpour.Bencode.Encoder.encode
    |> SHA1.HashData
    |> InfoHash
...
let parse (bytes: byte[]) : TorrentMetaInfo =
    ...
    { Announce     = dictGetReq "announce" root |> asString
      ...
      CreationDate = dictGet "creation date" root |> Option.map asInt |> Option.map DateTimeOffset.FromUnixTimeSeconds
      Info         = parseInfo infoVal
      InfoHash     = computeInfoHash infoVal }
