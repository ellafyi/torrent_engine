...
// Helper
let decodeStr (s: string) =
    decode (System.Text.Encoding.ASCII.GetBytes s)
...
let ``LIST-02 List of integers`` () =
    let expected =
        BencodeValue.List [ BencodeValue.Integer 1L; BencodeValue.Integer 2L; BencodeValue.Integer 3L ]
...
let ``LIST-03 List of strings`` () =
    let expected =
        BencodeValue.List [ BencodeValue.String "foo"B; BencodeValue.String "bar"B ]
...
let ``LIST-04 Mixed list`` () =
    let expected =
        BencodeValue.List [ BencodeValue.Integer 99L; BencodeValue.String "test"B ]
...
let ``DICT-02 Single key`` () =
    let expected =
        BencodeValue.Dictionary(Map.ofList [ ("foo"B, BencodeValue.Integer 1L) ])
...
let ``DICT-03 Multiple keys`` () =
    let expected =
        BencodeValue.Dictionary(Map.ofList [ ("bar"B, BencodeValue.Integer 2L); ("foo"B, BencodeValue.Integer 1L) ])
...
let ``DICT-04 String value`` () =
    let expected =
        BencodeValue.Dictionary(Map.ofList [ ("name"B, BencodeValue.String "alice"B) ])
...
let ``CMB-03 Nested dict`` () =
    let inner =
        BencodeValue.Dictionary(Map.ofList [ ("inner"B, BencodeValue.Integer 42L) ])
...
let ``CMB-04 Torrent-like info dict`` () =
    ...
    Assert.Equal(
        Ok expected,
        decodeStr
            "d6:lengthi1048576e4:name8:test.txt12:piece lengthi262144e6:piecesl20:aaaaaaaaaaaaaaaaaaaa20:bbbbbbbbbbbbbbbbbbbbee"
    )

[<Fact>]
let ``CMB-05 Deeply nested structure`` () =
    let innerDict =
        BencodeValue.Dictionary(Map.ofList [ ("b"B, BencodeValue.Integer 2L) ])

    let expected =
        BencodeValue.Dictionary(Map.ofList [ ("a"B, BencodeValue.List [ BencodeValue.Integer 1L; innerDict ]) ])
...
let ``CMB-06 List of mixed dicts`` () =
    let dict1 =
        BencodeValue.Dictionary(
            Map.ofList [ ("type"B, BencodeValue.Integer 0L); ("value"B, BencodeValue.String "foo"B) ]
        )

    let dict2 =
        BencodeValue.Dictionary(
            Map.ofList [ ("type"B, BencodeValue.Integer 1L); ("value"B, BencodeValue.String "bar"B) ]
        )
...
let ``CMB-07 Empty collections nested`` () =
    ...
    let dict1 =
        BencodeValue.Dictionary(Map.ofList [ (Array.empty, BencodeValue.List []) ])
...
let ``EDGE-01 Trailing data after valid value`` () =
    ...
    match result with
    | Ok(BencodeValue.Integer 1L, rem) -> Assert.Equal<byte array>("i2e"B, rem.ToArray())
