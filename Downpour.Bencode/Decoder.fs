...
and parseIntDigits (mem: ReadOnlyMemory<byte>) : ParseResult =
    match mem with
    | MemRem('0'B, MemRem('e'B, rest)) -> Ok(BencodeValue.Integer 0L, rest) // 0 allowed only when alone
    | MemRem('0'B, _) -> Error "Leading zeros are not permitted in integers"
...
and parseIntAcc (acc: int64) (mem: ReadOnlyMemory<byte>) : ParseResult =
    ...
    | MemRem(digit, rest) when digit >= '0'B && digit <= '9'B -> parseIntAcc (acc * 10L + int64 (digit - '0'B)) rest
    | MemNil -> Error "Unexpected EOF when parsing integer"
    | MemRem(bad, _) -> Error $"Invalid character in integer: {bad}"
...
/// Lists
and parseList
    (acc: BencodeValue list)
    (mem: ReadOnlyMemory<byte>)
    : Result<BencodeValue * ReadOnlyMemory<byte>, string> =
    match mem with
...
/// Dicts
and parseDict
    (acc: Map<byte array, BencodeValue>)
    (mem: ReadOnlyMemory<byte>)
    : Result<BencodeValue * ReadOnlyMemory<byte>, string> =
    match mem with
